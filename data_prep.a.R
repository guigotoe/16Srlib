...
## Options ##
pa <- '/home/torres/ikmb_storage/Mangrove/16Sfa/2016/mothur_gg/'#
ra <- '/home/torres/ikmb_storage/Mangrove/16Sfa/08_2017_results/2016/2017_11_gg2/'
...
option_list <- list(
  make_option(c("-B","--biom"),action="store",type="character",default=paste(pa,'16s_OTU_table.biom',sep=''),
...
if (!is.na(opt$biom)){
  data <- mothur.biom(opt$biom,opt$metadata)
}else{
...
packages("Biostrings")
try(OTUs.fa <- readDNAStringSet(opt$fasta))
for (i in 1:2){
  if(i==1){
  ...
  genus <- length(levels(as.factor(fData(data.f)$Genus)))
  if (!is.na(opt$biom)){otu <- length(levels(as.factor(fData(data.f)$Specie)))}else otu <- length(levels(as.factor(fData(data.f)$OTU)))
  ## Normalizing
  message("Normalizing...")
  ...
  ## Fasta files
  try(if(names(OTUs.fa)[1]!=as.character(fData(data.f)$OTU)[1]) names(OTUs.fa) <- gsub("Otu","Otu0",names(OTUs.fa),fixed=T))
  try(selected.fa <- OTUs.fa[names(OTUs.fa)%in%as.character(fData(data.f)$OTU)])
  try(writeXStringSet(selected.fa,paste(opt$out,'OTU_',fname,'.fa',sep=''),format="fasta"))
  ###
  ...
  output <- paste(paste("Filtred mode ",fname,":\n An OTU was kept when its ",avg,"count was grater than ",otu.coverage," reads ",shared.prc,
                        ...
                        " - Information: ",sum(fData(data)$Size)," Total reads;"," retained: ",round(100*retained.info,2),"%; lost: ",round((100-100*retained.info),2),"%",sep=""),"\n",
